...
Flow:
  ...
  4. call_claude()          — Call Anthropic API → get structured JSON itinerary
...
# ── Constants ──────────────────────────────────────────────────────────────────
FLIGHTS_CSV       = "flights_intl_v2.csv"
HOTELS_CSV        = "accommodation_intl_v2.csv"
ACTIVITIES_CSV    = "activities_intl_v2.csv"
...
def parse_user_request(user_input: str, origin_city: str = "Sydney") -> dict:
    ...
    # Budget
    budget_match = re.search(r'\$?\s*([\d,]+)\s*(?:aud|dollars?)?', text)
    budget_aud = int(budget_match.group(1).replace(',', '')) if budget_match else None
...
def query_inventory(params: dict) -> dict:
    ...
    for city in destinations:
        a = activities_df[
            ...
            (activities_df["group_size_max"] >= group_size)
        ]
        # Theme matching: boost activities with matching theme_tags
        theme_mask = a["theme_tags"].str.contains(theme, na=False)
...
def parse_claude_response(raw: str) -> dict:
    """Extract JSON from Claude's response, handling edge cases."""
    raw = raw.strip()
